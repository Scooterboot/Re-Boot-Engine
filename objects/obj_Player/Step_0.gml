...
if(!global.GamePaused())
{
	...
	state != State.Slide && state != State.Dodge && state != State.Spark && state != State.BallSpark && 
...
#region Liquid State
	...
	if(instance_exists(findLiquid))
	{
		...
		if(liquidLevel > dph)
		{
			_liqType = findLiquid.liquidType;
			if(!item[Item.GravitySuit])
			{
				if(item[Item.HydroBoost] && _liqType == LiquidType.Water)
				{
					liquidState = LiquidState.HydroBoost;
				}
				else if(_liqType == LiquidType.Water)
				{
					liquidState = LiquidState.Liquid;
				}
				if(_liqType == LiquidType.Lava || _liqType == LiquidType.Acid)
				{
					liquidState = LiquidState.DmgLiquid;
				}
			}
		}
	}
...
#endregion
#region Dodge activation/recharge logic
	var dRechargeMax = dodgeChargeCellSize * dodgeChargeCells,
		dRechargeMax2 = dRechargeMax-dodgeRechargeRate;
	if(grounded || state == State.Grip || state == State.Grapple || state == State.GravGrapple || ((state == State.Spark || state == State.BallSpark) && shineEnd > 0))
	{
		dRechargeMax2 = dRechargeMax;
	}
	if(dodgeRecharge < dRechargeMax2 && state != State.Dodge)
	{
		dodgeRecharge = min(dodgeRecharge+dodgeRechargeRate, dRechargeMax2);
	}
	...
	canDodge = (dodgeCharge >= dodgeChargeCellSize);
	
	var _dodgeType = 0;
	if(canDodge && dir != 0 && (state == State.Stand || state == State.Crouch || state == State.Jump || state == State.Somersault || (state == State.Grip && !startClimb) || state == State.Dodge))
	{
...
#region Power Grip Logic

	#region Grip Collision

	if(item[Item.PowerGrip] && (state == State.Jump || state == State.Somersault) && !grounded && velY >= 0 && move2 == dir && dir != 0 && self.entity_place_meeting(move2,0))
	{
		if(!self.entity_place_collide(0,-4) && ((state == State.Jump && !self.entity_place_collide(0,3)) || (state == State.Somersault && !self.entity_place_collide(0,12))) && self.CanChangeMask(mask_Player_Grip))
		{
			var canGrip = false;
			...
			var lCheck = x,
				rCheck = self.bb_right(x)+1 + 1;
			if(move2 == -1)
			{
				lCheck = self.bb_left(x)-1;
				rCheck = x;
			}
			var bTopNew = y - sprite_get_yoffset(mask_Player_Grip);
			var bTopDiff = abs(self.bb_top(y) - bTopNew);
			
			if(canGrip && 
				self.entity_collision_rectangle(lCheck, bTopNew, rCheck, self.bb_top(y)+1+ceil(velY)) &&
				!self.entity_collision_rectangle(lCheck, bTopNew-3, rCheck, bTopNew-1))
			{
				var slopeLX = self.bb_right(x) + 1,
					slopeLY = self.bb_top(y),
					slopeRX = self.bb_right(x)+8 + 1,
					slopeRY = self.bb_top(y)-8;
				if(move2 == -1)
				{
					slopeLX = self.bb_left(x)-8;
					slopeLY = self.bb_top(y)-8;
					slopeRX = self.bb_left(x);
					slopeRY = self.bb_top(y);
				}
				var slopeOffset = 0;
				while(self.entity_collision_rectangle(lCheck, self.bb_top(y)-1+slopeOffset, rCheck, self.bb_top(y)+slopeOffset) && slopeOffset >= -bTopDiff)
				{
					slopeOffset -= 1;
				}
				if(self.entity_collision_line(slopeLX, slopeLY-2+slopeOffset, slopeRX, slopeRY-2+slopeOffset))
				{
					canGrip = false;
				}
				...
				if(canGrip)
				{
					...
					self.ChangeState(State.Grip, AnimState.Grip, MoveState.Custom, mask_Player_Grip, false);
					with(obj_Camera)
					{
						stallY = true;
					}
					
					shiftY += (scr_ceil(position.Y) - position.Y);
					for(var i = 0; i < 4+bTopDiff; i++)
					{
						if(self.entity_collision_rectangle(lCheck, self.bb_top()-2+shiftY, rCheck, self.bb_top()-1+shiftY))
						{
							shiftY -= 1;
						}
						if(!self.entity_collision_rectangle(lCheck, self.bb_top()+shiftY, rCheck, self.bb_top()+1+shiftY))
						{
							shiftY += 1;
						}
					}
	...
	#endregion
	#region Climb activation/counter
	...
	if(state == State.Grip)
	{
		...
		if(climbIndexCounter >= 1/_liqMult || climbIndex <= 1)
		{
			if(state == State.Grip && startClimb)
			{
				var ciNum = 1;
				if(liquidState == LiquidState.None && climbIndex >= 10 && (cSprint || global.autoSprint) && move != 0)
				{
					ciNum = 2;
				}
				climbIndex = min(climbIndex + ciNum, 18);
			}
			climbIndexCounter = max(climbIndexCounter - (1/_liqMult), 0);
		}
	...
	var qc_flag_1 = false,
		qc_flag_2 = false;
	if(global.quickClimb && item[Item.PowerGrip] && dir != 0 && self.entity_place_collide(2*dir,0) && !self.entity_place_collide(0,0))
	{
		qc_flag_1 = ((state == State.Stand || state == State.Crouch) && grounded && move2 == dir);
		//qc_flag_2 = (state == State.Dodge && sign(velX) == dir); // EXPERIMENTAL
	}
	if(qc_flag_1 || qc_flag_2)
	{
		var canGrip = false;
		var num = instance_place_list(x+2*move2,y,solids,blockList,true);
		if(num > 0)
		{
			for(var j = 0; j < num; j++)
			{
		...
		{
			var qcHeight = 0;
			var bbottom = self.bb_bottom(y);
			var heightMax = self.bb_height()+10;
			...
			for(var i = 0; i < 2; i++)
			{
				...
				{
					while(qcHeight > -heightMax && self.entity_collision_rectangle(lCheck,bbottom+qcHeight,rCheck,bbottom+qcHeight+1))
					{
						qcHeight--;
					}
				}
				...
				{
					qcHeight = -heightMax;
					while(qcHeight < -5 && self.entity_collision_rectangle(lCheck,bbottom+qcHeight,rCheck,bbottom+qcHeight+1))
					{
						qcHeight++;
					}
					while(qcHeight < -5 && !self.entity_collision_rectangle(lCheck,bbottom+qcHeight,rCheck,bbottom+qcHeight+1))
				...
				if(qcHeight <= -7)
				{
					...
					if(self.entity_collision_rectangle(lCheck,yHeight,rCheck,yHeight+1) && !self.entity_collision_line(slopeLX,slopeLY,slopeRX,slopeRY))
					{
						var slopeOffset = 0;
						while(slopeOffset > -16 && self.entity_collision_rectangle(lCheck,yHeight+slopeOffset,rCheck,yHeight+slopeOffset+1))
						{
							slopeOffset -= 1;
						}
					...
					if(!self.entity_collision_rectangle(lCheck,yHeight-15,rCheck,yHeight-2))
					{
			...
			if(qcHeight <= -7 && quickClimbTarget > 0 && ((qc_flag_1 && cJump && rJump) || qc_flag_2))
			{
				jump = 0;
				fVelY = 0;
				velY = 0;
				...
				self.ChangeState(State.Grip, AnimState.Grip, , mask_Player_Crouch, false);
...
#region Horizontal Movement
	...
	if(state == State.Morph)
	{
		if(grounded)
		{
			if(abs(velX) > maxSpeed[MaxSpeed.MorphBall,liquidState] && (liquidState == LiquidState.None || liquidState == LiquidState.HydroBoost))
			{
	...
	else if(state == State.Somersault || animState == AnimState.DmgBoost)
	{
	...
	{
		if(sprint)// && (liquidState == LiquidState.None || liquidState = LiquidState.HydroBoost))
		{
			if(item[Item.SpeedBooster] && liquidState == LiquidState.None)
			{
	...
	fMoveSpeed = moveSpeed[MoveSpeed.Default,liquidState];
	if(state == State.Morph)
	{
	...
	if(maxSpdInd == MaxSpeed.Sprint || maxSpdInd == MaxSpeed.SpeedBoost)
	{
		var runMaxSpd = maxSpeed[MaxSpeed.Run,liquidState],
			...
			runMoveSpd = moveSpeed[MoveSpeed.Default,liquidState],
	...
	fFrict = frict[Friction.Default,liquidState];
	if(!grounded)
	{
		fFrict = frict[Friction.Air,liquidState];
	}
	...
	{
		var _move = 0,
			_moveSpeed = fMoveSpeed;
		if(!brake)
		{
			_move = move;
			if(sign(dirFrame) != dir && sign(dirFrame) != 0 && !sprint)
			{
				_moving = 0;
				if(sign(velX) != -dir)
				{
		...
		if(cAimLock)
		{
			_frict = fMoveSpeed + fFrict + sprintTurnSpeed[liquidState];
		}
		...
		{
			_move = 0;
			_frict = max(_frict, fMoveSpeed + fFrict);
		}
		...
		self.PerformMovement(_move, _moveSpeed, turnaroundSpd, _frict, fMaxSpeed);
	}
	if(moveState == MoveState.Somersault)
	{
		var _move = move;
		var spinJumpMoveFlag = (move == 0 && state == State.Somersault && (frame[Frame.Somersault] >= 2 || abs(velX) > 2*moveSpeed[MoveSpeed.Default,liquidState]));
		...
		{
			_move = 0;
			_frict = fMoveSpeed + fFrict;
		}
		
		self.PerformMovement(_move, fMoveSpeed, 0, _frict, fMaxSpeed);
	}
	if(moveState == MoveState.Slide)
	{
		if(slide <= 0)
		{
			self.PerformMovement(0, 0, 0, frict[Friction.Slide,liquidState], 0);
		}
	}
	if(moveState == MoveState.Crouch)
	{
		_moving = 0;
		var _frict = frict[Friction.Crouch,liquidState];
		if(abs(velX) > maxSpeed[MaxSpeed.Sprint,liquidState])
		{
			_frict += max(abs(velX) - maxSpeed[MaxSpeed.Sprint,liquidState], 0) / 3;
		}
		
		self.PerformMovement(0, 0, 0, _frict, 0);
	}
	if(moveState == MoveState.MoonPrep)
	{
		_moving = 0;
		self.PerformMovement(0, 0, 0, frict[Friction.MoonPrep,liquidState], 0);
	}
	
	if(moveState != MoveState.Custom)
	{
		var _frict = frict[Friction.SpdCap,liquidState];
		var maxSpeed2 = maxSpeed[MaxSpeed.SpeedBoost,liquidState];
		if(abs(velX) > maxSpeed2 && grounded)
		{
			if(sign(velX) == 1)
			{
				velX = max(velX - _frict, maxSpeed2);
			}
			if(sign(velX) == -1)
			{
				velX = min(velX + _frict, -maxSpeed2);
			}
		}
	}
	...
	{
		if(state == State.Stand && grounded && liquidState == LiquidState.None && !brake && ((velX != 0 && sign(velX) == dir) || (prevVelX != 0 && sign(prevVelX) == dir)))
		{
	...
	if(item[Item.SpeedBooster])
	{
		...
		else if(!speedBoost && (abs(velX) >= minBoostSpeed || abs(spiderSpeed) >= minBoostSpeed) && (state == State.Stand || !_RUN_RESTRICT_SB) && state != State.Dodge && state != State.Grapple && !grapBoost && !spiderJumpBoost && liquidState == LiquidState.None)
		{
...
#region Vertical Movement
	...
	if(moonFallState || moonFall || state == State.Grip)
	{
		bufferJump = 0;
		coyoteJump = 0;
	}
	...
	if(liquidState != LiquidState.None)
	{
	...
	{
		if(jump > 0 || velY > 0 || item[Item.GravitySuit])
		{
			liqGravShift = 0;
		}
	...
	if(isJumping)
	{
		...
		if(jump <= 0)
		{
			if(shineCharge > 0 && rJump && (self.CanChangeMask(mask_Player_Jump) || state == State.Morph) && !self.entity_place_collide(0,-1) && !moonFallState && 
			...
			(state != State.Morph || (state == State.Morph && ((item[Item.SpringBall] && morphFrame <= 0) || ((unmorphing > 0 || morphSpinJump) && self.CanChangeMask(mask_Player_Somersault))))) && state != State.DmgBoost)
			{
				if((grounded && !moonFallState) || coyoteJump > 0 || canWallJump || (state == State.Grip && canGripJump) || 
				(item[Item.SpaceJump] && velY >= sjThresh && state == State.Somersault && liquidState == LiquidState.None && (!detectWJ || bufferJump <= 1) && rRespinJump))//&& ((!detectWJ && rRespinJump) || bufferJump <= 1)))
				{
					...
					jump += jumpMax[_jInd, liquidState];
					...
					if((rJump || bufferJump > 0) && state != State.Morph)
					{
						...
						if((abs(velX) > 0 && !walkState) || (move != 0 && !walkState) || cSprint || (!grounded && state != State.Grip) || (state == State.Crouch && !self.CanChangeMask(mask_Player_Jump)))
						{
							var mask = mask_Player_Jump;
							if(!self.CanChangeMask(mask_Player_Jump))
							{
						...
						{
							self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, false, false);
						}
					}
			...
			if(abs(velX) > maxSpeed[MaxSpeed.Sprint, LiquidState.None])
			{
				fJumpSpeed += max((abs(velX) - maxSpeed[MaxSpeed.Sprint, LiquidState.None]) / 2, 0);
				prevLiqJSpd += max((abs(velX) - maxSpeed[MaxSpeed.Sprint, LiquidState.None]) / 2, 0);
			}
			
			if(jumping && jump <= 0 && velY < 0 && liquidState == LiquidState.None && prevLiquidState != liquidState && !item[Item.GravitySuit])
			{
	...
	if(bombJump > 0)
	{
		if(spiderBall && (spiderEdge != Edge.None || spiderJump))
		{
			if(liquidState == LiquidState.None || liquidState == LiquidState.HydroBoost)
			{
...
#region Grapple Movement
	...
	if(self.GrappleSwinging())
	{
		...
		var reel = 0;
		var _lmult = 1;
		if(liquidState == LiquidState.HydroBoost)
		{
			_lmult = 0.75;
		}
		...
		if(state == State.Grapple)
		{
			...
			else
			{
				...
				if(!speedBoost)
				{
					...
					velX *= grapDamp[liquidState];
					velY *= grapDamp[liquidState];
				}
				else
				{
...
#region Grav Grapple Movement
	...
	if(self.GravGrapSwinging())
	{
		...
		if(move != 0)
		{
			...
			gravGrapAngleVel = clamp(gravGrapAngleVel + _spd*_lmult*move, -gravGrapAngleMax*_lmult, gravGrapAngleMax*_lmult);
		}
		else
		{
		...
		var _vX = (normX - position.X) / (gravGrapDist+100), //(normX - position.X)/250,
			_vY = (normY - position.Y) / (gravGrapDist+100); //(normY - position.Y)/250;
		
		velX += _vX * _lmult;
		velY += _vY * _lmult;
		
		var _vDecay = gravGrapVelDecay;
		if(liquidState != LiquidState.None)
		{
			_vDecay -= 0.03;
		}
...
#region Spider Ball Movement
	if(spiderBall)
	{
		...
		else
		{
			...
			{
				if(self.entity_place_collide(0,-2)) { spiderEdge = Edge.Top; }
				else if(self.entity_place_collide(-2,0)) { spiderEdge = Edge.Left; }
				else if(self.entity_place_collide(2,0)) { spiderEdge = Edge.Right; }
				else { spiderEdge = Edge.None; }
			}
			if(spiderEdge == Edge.Top && !self.entity_place_collide(0,-2))
			{
				if(self.entity_place_collide(0,2)) { spiderEdge = Edge.Bottom; }
				else if(self.entity_place_collide(-2,0)) { spiderEdge = Edge.Left; }
				else if(self.entity_place_collide(2,0)) { spiderEdge = Edge.Right; }
				else { spiderEdge = Edge.None; }
			}
			if(spiderEdge == Edge.Left && !self.entity_place_collide(-2,0))
			{
				if(self.entity_place_collide(2,0)) { spiderEdge = Edge.Right; }
				else if(self.entity_place_collide(0,-2)) { spiderEdge = Edge.Top; }
				else if(self.entity_place_collide(0,2)) { spiderEdge = Edge.Bottom; }
				else { spiderEdge = Edge.None; }
			}
			if(spiderEdge == Edge.Right && !self.entity_place_collide(2,0))
			{
				if(self.entity_place_collide(-2,0)) { spiderEdge = Edge.Left; }
				else if(self.entity_place_collide(0,-2)) { spiderEdge = Edge.Top; }
				else if(self.entity_place_collide(0,2)) { spiderEdge = Edge.Bottom; }
				else { spiderEdge = Edge.None; }
			}
			...
			if(abs(spiderSpeed) > maxSpeed2)
			{
				...
				{
					spiderSpeed = max(spiderSpeed - _frict, maxSpeed2);
				}
				if(sign(spiderSpeed) == -1)
				{
					spiderSpeed = min(spiderSpeed + _frict, -maxSpeed2);
				}
			}
...
#region Collision
	...
	if(state != State.Elevator && state != State.Recharge)
	{
		...
		if(state == State.Grip)
		{
			...
			if(startClimb)
			{
				...
				if(liquidState != LiquidState.None && climbIndex > 1)
				{
					cX *= liquidAnimMult[liquidState];
					cY *= liquidAnimMult[liquidState];
				}
				
				if(sign(velX) == sign(cX))
				{
					if(animState == AnimState.Morph && climbIndex > 7 && move == sign(cX))
					{
						velX = max(abs(cX),abs(velX))*sign(velX);
					}
					fVelX = max(abs(cX),abs(velX))*sign(velX);
				}
				else
				{
					fVelX = cX + velX;
				...
				if(cX != 0 && cY == 0)
				{
					if(self.entity_place_collide(dir,0) && !self.entity_place_collide(0,1))
					{
						while(self.entity_place_collide(dir,shiftY) && shiftY > -4)
						{
							shiftY--;
						}
					}
	...
	if(fastWJGrace)
	{
		...
		{
			var fwjFrict = fMoveSpeed+fFrict;
			if(velX > 0)
			{
...
#region Stand, Walk, Run, Sprint, Brake
	if(state == State.Stand)
	{
		...
		if(frame[Frame.Land] > 0)
		{
			landSlide = true;
		}
		
		if(frame[Frame.Crouch] <= 0)
		{
			var velMove = (((velX != 0 && sign(velX) == dir) || (prevVelX != 0 && sign(prevVelX) == dir)) && !landSlide),
			...
			{
				animState = AnimState.MoonPrep;
				moveState = MoveState.MoonPrep;
			}
			else if((velMove || moveMove) && frame[Frame.Land] <= 0)
			{
		...
		if(!grounded && dir != 0)
		{
			self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, ledgeFall);
		}
		...
		if(instance_exists(pushBlock) && isPushing)
		{
			...
			var animRate = 0.275 * liquidAnimMult[liquidState];
			if(cSprint || global.autoSprint)
			{
				animRate = 0.375 * liquidAnimMult[liquidState];
			}
...
#region Slide
	if(state == State.Slide)
	{
		...
		if(abs(velX) <= fFrict)
		{
			...
			if(!self.CanChangeMask(mask_Player_Crouch))
			{
				mask = mask_Player_Somersault;
			}
		...
		if(!grounded && self.CanChangeMask(mask_Player_Jump))
		{
			self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, false);
		}
		// wip
	}
	else
...
#region Crouch
	if(state == State.Crouch)
	{
		...
		if(((cPlayerUp && rPlayerUp) || uncrouch >= 7) && self.CanChangeMask(mask_Player_Stand))
		{
			aimAnimDelay = aimAnimDelayMax;
			self.ChangeState(State.Stand, AnimState.Stand, MoveState.Default, mask_Player_Stand, true);
			frame[Frame.Crouch] = 5-frame[Frame.Crouch];
		}
		if(item[Item.MorphBall] && frame[Frame.Crouch] <= 3 && ((cPlayerDown && rPlayerDown && move2 == 0) || (cMorph && rMorph)) && animState != AnimState.Morph && morphFrame <= 0)
		{
		...
		else if(!grounded && self.CanChangeMask(mask_Player_Jump))
		{
			self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, false);
		}
	}
...
#region Morph Ball
	...
	if(item[Item.MorphBall])
	{
			...
			state != State.Crouch && state != State.Spark && state != State.BallSpark && state != State.Grip && state != State.GravGrapple)
		{
			...
			var oldY = y;
			self.ChangeState(State.Morph, AnimState.Morph, MoveState.Default, mask_Player_Morph, grounded);
			morphFrame = 8;
			morphYDiff = y-oldY;
		}
		
		if (dir != 0 && state == State.Jump && animState != AnimState.Morph && morphFrame <= 0 && aimAngle == -2 && prevAimAngle == aimAngle &&
			cPlayerDown && rPlayerDown && !cPlayerUp && !cAimUp && !cAimDown && !cAimLock && move == 0)
		{
			audio_play_sound(snd_Morph,0,false);
			var oldY = y;
			self.ChangeState(State.Morph, AnimState.Morph, MoveState.Default, mask_Player_Morph, grounded);
	...
	if(state == State.Morph)
	{
		...
		if(grounded && !prevGrounded)
		{
			if(morphFrame <= 0 && !shineRampFix && !slopeGrounded)
			{
				if(!self.SpiderActive())
				{
					...
					if((_SPEED_KEEP == 0 || (_SPEED_KEEP == 2 && liquidState != LiquidState.None)) && canMorphBounce && !justBounced)
					{
						velX = min(abs(velX),maxSpeed[MaxSpeed.Run,liquidState])*sign(velX);
						speedCounter = 0;
						speedBoost = false;
		...
		if((self.CanChangeMask(mask_Player_Crouch) || _CRYSTAL_CLIP) && energy < lowEnergyThresh && ammo > 0 && cFlashStartCounter > 0 && cFire && cPlayerDown)
		{
			cFlashStartCounter++;
		...
		if(prevSpiderEdge == Edge.None)
		{
			if(morphFrame <= 0 && !shineRampFix)
			{
				...
				if(grounded && !prevGrounded)
				{
					...
					if(!spiderGrappleSpeedKeep && (_SPEED_KEEP == 0 || (_SPEED_KEEP == 2 && liquidState != LiquidState.None)))
					{
...
#region Jump
	if(state == State.Jump)
	{
		...
		if(frame[Frame.DmgBoost] <= 0 || frame[Frame.DmgBoost] >= 19 || breakSpinJump)
		{
			...
			else if(self.CanChangeMask(mask_Player_Jump))
			{
				self.ChangeState(state, animState, moveState, mask_Player_Jump, false);
			}
		}
		else if(animState == AnimState.DmgBoost)
		{
			if(self.CanChangeMask(mask_Player_Somersault))
			{
				self.ChangeState(state, animState, moveState, mask_Player_Somersault, false);
			}
		}
		...
		if(grounded)
		{
			...
			tempDisableBoostCheck = true;
			tempDisableSpeedCheck = true;
			tempDisableScrewCheck = true;
			if(mask_index == mask_Player_AimDown)
			{
				if(!self.CanChangeMask(mask_Player_Stand))
				{
...
#region Somersault
	if(state == State.Somersault)
	{
		...
		else
		{
			...
			if(!audio_is_playing(snd_Charge) && !audio_is_playing(snd_Charge_Loop))
			{
				...
				if(somerUWSndCounter < 0)
				{
					...
					somerUWSndCounter = 33 / (2*liquidAnimMult[liquidState]);
				}
			}
		}
		...
		{
			if(prAngle || (cPlayerUp && rPlayerUp) || (cPlayerDown && rPlayerDown) || breakSpinJump)//(cFire && rFire) || (!cFire && !rFire && CanCharge()) || (!cFire && rFire && statCharge >= 20))
			{
				tempDisableBoostCheck = true;
				tempDisableSpeedCheck = true;
				tempDisableScrewCheck = true;
				...
				else
				{
					...
					if(aimAngle == -2 || aimFrame <= -3)
					{
						mask = mask_Player_AimDown;
					}
					self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask, false);
				}
			}
		}
	}
...
#region Power Grip
	if(state == State.Grip)
	{
			...
			if(climbIndex > 0)
			{
				...
				if(animState != AnimState.Morph)
				{
					...
					if(climbIndex >= 3 && climbTarget == 1)
					{
						audio_play_sound(snd_Morph,0,false);
						var oldY = y;
						self.ChangeState(state, AnimState.Morph, MoveState.Default, mask_Player_Morph, true);
						morphFrame = 8;
						morphYDiff = y-oldY;
					}
		...
		else
		{
			...
			animState = AnimState.Grip;
			mask_index = mask_Player_Grip;
			
			climbTarget = 0;
			if(!self.entity_place_collide(0,-8))
			{
				var ctStart = 0;
				while(ctStart >= -16 && self.entity_collision_rectangle(x, self.bb_top(y)+ctStart, x+9*_dir, self.bb_top(y)+1+ctStart))
				{
					ctStart--;
				}
				if(ctStart >= -16)
				{
					...
					while(ctHeight <= -morphH && self.entity_collision_rectangle(x, self.bb_top(y)+ctStart+ctHeight, x+9*_dir, self.bb_top(y)+1+ctStart+ctHeight))
					{
						ctHeight++;
					}
					if(ctHeight <= -crouchH)
			...
			if(grounded)
			{
				self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, true);
			}
			...
			if(!self.entity_collision_rectangle(lCheck, self.bb_top(y), rCheck, self.bb_top(y)+1))
			{
				breakGrip = true;
			}
		}
		...
		if(self.entity_collision_line(slopeLX, slopeLY-2, slopeRX, slopeRY-2))
		{
			colFlag = true;
		}
		...
		if((!self.entity_place_collide(2*_dir,0) && !self.entity_place_collide(2*_dir,2) && !self.entity_place_collide(0,2)) || (self.entity_collision_rectangle(lCheck, self.bb_top(y)-2, rCheck, self.bb_top(y)-1) && !startClimb) || (colFlag && !startClimb) || (cPlayerDown && cJump && rJump) || (place_meeting(x,y,ColType_MovingSolid) && !startClimb))
		{
			breakGrip = true;
		}
		if(breakGrip)
...
#endregion
#region Dodge (Accel Dash / Hydro Boost)
	...
	if(_dodgeType > 0)
	{
		if(cDodge && rDodge)
		{
			...
			dodgeLength = 0;
			dodgeLengthEnd = dodgeLenEnd[dodgeType];
			dodgeLengthMax = dodgeLenMax[dodgeType];
			
			var _oldDir = dodgeDir;
			if(dodgeType == 1)
			{
				if(move2 == dir)
				{
			...
			else if(dodgeType >= 2)
			{
				...
				if(move2 != 0 || aUp || aDown)
				{
					...
					else if(aDown || cPlayerDown)
					{
						...
						if(move2 != 0)
						{
							dodgeDir = _downDir*move2;
						}
						else
						{
							dodgeDir = _downDir*dir;
						}
					}
					else
					{
	...
	if(state == State.Dodge)
	{
		...
		var shoot = _DASH_SHOOT_CANCEL && ((cFire && rFire) || (!cFire && !rFire && self.CanCharge()));
		if(dodgeLength >= dodgeLengthMax || shoot)
		{
			...
			if(grounded)
			{
				if(!self.CanChangeMask(mask_Player_Stand) || groundedDodge == 2)
				{
					self.ChangeState(State.Crouch, AnimState.Crouch, MoveState.Crouch, mask_Player_Crouch, true);
				}
				else
		...
		else
		{
			...
			if(dodgeLength < dodgeLengthEnd)
			{
				...
				var velMax = max_spd_d;
				if(velCheck >= max_spd_d && (liquidState == LiquidState.None || dodgeType >= 3) && (dodgeLength <= 0 || velCheck > max_spd_d) && velCheck <= max_spd_s)
				{
					velMax = max_spd_s;
				}
				
				var velMaxX = lengthdir_x(velMax, dodgeDir-90),
					velMaxY = lengthdir_y(velMax, dodgeDir-90);
...
#region Shine Spark
	if(state == State.Spark || state == State.BallSpark)
	{
		...
		else
		{
			...
			#region Spark Steering
			shineDirDiff = 0;
			if(_SPARK_STEERING > 0)
			{
				var moveY = (cPlayerDown-cPlayerUp);
				
				var angleChange = _SPARK_STEERING;
			...
			var _shineDir = self.GetSparkDir();
			#region Spark sliding angle tweak
			var destAngDif = 0;
			
			var xdir = 0;
			if(!self.SparkDir_VertUp() && !self.SparkDir_VertDown())
			{
				xdir = sign(_shineDir);
			}
			if(xdir != 0)
			{
				...
				{
					destAngDif = angle_difference(edgeAngle[Edge.Bottom]+90*xdir, _shineDir);
				}
				if(self.entity_place_collide(0,-2))
				{
					destAngDif = angle_difference(edgeAngle[Edge.Top]-90*xdir, _shineDir);
				}
			}
			
			var ydir = 0;
			if(self.SparkDir_VertUp() || self.SparkDir_DiagUp())
			{
				ydir = -1;
			}
			if(self.SparkDir_VertDown() || self.SparkDir_DiagDown())
			{
				ydir = 1;
			}
			if(ydir != 0)
			{
				...
				{
					destAngDif = angle_difference(edgeAngle[Edge.Right]-90*ydir, _shineDir);
				}
				if(self.entity_place_collide(-2,0))
				{
					destAngDif = angle_difference(edgeAngle[Edge.Left]+90*ydir, _shineDir);
				}
			}
			
			if(destAngDif == 0 || sign(shineAngleTweak) != sign(destAngDif))
			{
				shineAngleTweak = 0;
			}
			if(shineAngleTweak < destAngDif)
			{
				shineAngleTweak = min(shineAngleTweak+1, destAngDif);
			}
			if(shineAngleTweak > destAngDif)
			{
				shineAngleTweak = max(shineAngleTweak-1, destAngDif);
			}
			#endregion
			
			velX = lengthdir_x(shineSparkSpeed, _shineDir-90 + shineAngleTweak);
			velY = lengthdir_y(shineSparkSpeed, _shineDir-90 + shineAngleTweak);
			
			#region Reflec panel logic
			...
			shineSparkSpeed = min(shineSparkSpeed+moveSpeed[MoveSpeed.SparkAcc,liquidState], maxSpeed[MaxSpeed.ShineSpark,liquidState]);
	...
	if(state == State.Hurt)
	{
		...
		if(hurtTime <= 0)
		{
				...
				lastState == State.Grip || lastState == State.Spark || lastState == State.Grapple || lastState == State.Dodge)
			{
				...
				if(self.CanChangeMask(mask_Player_Jump))
				{
					self.ChangeState(State.Jump, AnimState.Jump, MoveState.Default, mask_Player_Jump, false, true);
				}
				else if(self.CanChangeMask(mask_Player_Somersault))
				{
...
#region Damage Boost
	...
	if(state == State.Hurt || state == State.Stand || (state == State.Crouch && self.CanChangeMask(mask_Player_Stand)) || state == State.Jump || state == State.Somersault)
	{
		if(dmgBoost > 0 && animState != AnimState.Morph && cJump && move2 == -dir)
		{
			self.ChangeState(State.DmgBoost, AnimState.DmgBoost, MoveState.Default, mask_Player_Somersault, false);
			dmgBoost = 0;
		}
	}
	else
	...
	if(state == State.DmgBoost)
	{
		...
		if(frame[Frame.DmgBoost] >= 19 || breakSpinJump)
		{
			animState = AnimState.Jump;
			if(self.CanChangeMask(mask_Player_Jump))
			{
...
#region Aim Control, part 2
	...
	if(instance_exists(lifeBoxes[0]))
	{
		...
		lifeBoxes[0].offsetX = 0;
		if(state == State.Grip && !startClimb)
		{
			lifeBoxes[0].mask_index = mask_Player_Jump;
			lifeBoxes[0].offsetX = -3*grippedDir;
		}
		lifeBoxes[0].UpdatePos(x,y);
	}
	
	tempDisableBoostCheck = false;
	tempDisableSpeedCheck = false;
	tempDisableScrewCheck = false;
}
...
if(global.pauseState == PauseState.None || (self.VisorSelected(Visor.XRay) && global.pauseState == PauseState.XRay) || global.pauseState == PauseState.RoomTrans)
{
	...
	if(recoil)
	{
		recoilCounter = 4 + (statCharge >= maxCharge) + (liquidState != LiquidState.None);
		var aimClamp = 2 * _liqMult;
		aimFrame = clamp(aimFrame, aimFrameTarget-aimClamp,aimFrameTarget+aimClamp);
		aimSnap = 8;
		recoil = false;
	}
	
	var turnSpeed = 1 * _liqMult;
	if(dir == 0)
	{
		turnSpeed /= 2;
	}
	...
	else if(abs(dirFrameF) < 4 && animState != AnimState.Somersault && animState != AnimState.Morph && animState != AnimState.Grip && (animState != AnimState.Spark || shineRestart) && animState != AnimState.Grapple && animState != AnimState.GravGrapple && animState != AnimState.Dodge)
	{
		...
		else
		{
			...
			if(animState == AnimState.Stand && frame[Frame.Crouch] > 1)
			{
				sprtOffsetY = 11;
				legs = sprt_Player_TurnCrouchLeg;
			}
	...
	else
	{
		...
		switch animState
		{
			...
			case AnimState.Stand:
			{
				...
				if(self.VisorSelected(Visor.Scan) || self.VisorSelected(Visor.XRay) || aimFrame != 0 || frame[Frame.Land] > 0 || recoilCounter > 0 || runToStandFrame[0] > 0 || runToStandFrame[1] > 0 || walkToStandFrame > 0)
				{
					frame[Frame.Idle] = 0;
					frameCounter[Frame.Idle] = 0;
					...
					else
					{
						...
						if (aimFrame != aimFrameTarget && 
							((aimAngle == 2 && (lastAimAngle == 0 || (lastAimAngle == -1 && aimFrame >= 0))) ||
					...
					if(frame[Frame.Land] > 0)
					{
						...
						if(smallLand)
						{
							landFinal = smallLandSequence[scr_round(frame[Frame.Land])];
						}
						else
						{
							landFinal = landSequence[scr_round(frame[Frame.Land])];
						}
						legFrame = landFinal;
						...
						if(sign(velX) == dir)
						{
							frame[Frame.Land] = max(frame[Frame.Land] - (1 + max(1.5*(move != 0),abs(velX)*0.5)) * _liqMult, 0);
						}
						else
						{
							frame[Frame.Land] = max(frame[Frame.Land] - (1 * _liqMult), 0);
						}
					}
					else
					{
				...
				if(frame[Frame.Crouch] > 0)
				{
					var crouchFinal = crouchSequence[5 - scr_round(frame[Frame.Crouch])];
					legFrame = crouchFinal;
					sprtOffsetY = 11-crouchYOffset[legFrame];
					legs = sprt_Player_CrouchLeg;
					
					frame[Frame.Crouch] = max(frame[Frame.Crouch] - (1 + abs(velX)*0.5) * _liqMult, 0);
				}
			...
			case AnimState.Run:
			{
				...
				if(!roomTrans)
				{
					var num = clamp(4 * ((abs(velX)-maxSpeed[MaxSpeed.Run, LiquidState.None]) / (maxSpeed[MaxSpeed.SpeedBoost, LiquidState.None]-maxSpeed[MaxSpeed.Run, LiquidState.None])), speedCounter, 4);
					...
					if(!item[Item.SpeedBooster] || liquidState != LiquidState.None)
					{
						...
						if(abs(velX) > maxSpeed[MaxSpeed.Run, LiquidState.None])
						{
							num2 = lerp(num2, 2, (abs(velX)-maxSpeed[MaxSpeed.Run, LiquidState.None]) / (maxSpeed[MaxSpeed.Sprint, LiquidState.None]-maxSpeed[MaxSpeed.Run, LiquidState.None]));
						}
					}
					...
					if(!_SMOOTH_RUN_ANIM)
					{
						var runSpdMult = 1 - abs(velX)/maxSpeed[MaxSpeed.SpeedBoost, LiquidState.None];
					...
					else if(liquidState != LiquidState.None)
					{
						numCounter *= 2-_liqMult;
						numRun = 1;
					}
			...
			case AnimState.Brake:
			{
				...
				bodyFrame = clamp(5 - ceil(frame[Frame.Brake]/2), 0, 4);
				legFrame = bodyFrame;
			...
			case AnimState.Walk:
			{
				...
				var numCounter = 2.5 / _liqMult;
				if(abs(velX) > maxSpeed[MaxSpeed.MoonWalk,liquidState])
				{
					numCounter = 1.875 / _liqMult;
				}
				if(roomTrans)
				{
					numCounter = 5;
			...
			case AnimState.Push:
			{
	            ...
	            {
					var animSpStart = 0.375 * _liqMult,
						animSp = 0.275 * _liqMult;
					if(cSprint || global.autoSprint)
					{
						animSpStart = 0.5 * _liqMult;
						animSp = 0.375 * _liqMult;
					}
	            ...
	            {
	                frame[Frame.Push] = clamp(frame[Frame.Push] - (0.5 * _liqMult), 0, 3);
	            }
			...
			case AnimState.Crouch:
			{
				...
				if(aimFrame != 0 || frame[Frame.Crouch] > 0 || recoilCounter > 0 || self.VisorSelected(Visor.Scan) || self.VisorSelected(Visor.XRay))
				{
					frame[Frame.Idle] = 0;
					frameCounter[Frame.Idle] = 0;
					if(recoilCounter > 0 && aimFrame == (scr_round(aimFrame/2)*2) && transFrame <= 0)
					...
					if(frame[Frame.Crouch] > 0)
					{
						var crouchFinal = crouchSequence[scr_round(frame[Frame.Crouch])];
						legFrame = crouchFinal;
						
						frame[Frame.Crouch] = max(frame[Frame.Crouch] - 1 * _liqMult, 0);
					}
					else
					{
						legFrame = 0;
			...
			case AnimState.Morph:
			{
				...
				if(unmorphing > 0 && scr_round(morphFrame) <= 5)
				{
					...
					bodyFrame = clamp(scr_round(morphFrame)-1, 0, 4);
					yOffFrame = bodyFrame;
				}
				else if(scr_round(morphFrame) >= 4 && unmorphing == 0)
				{
					torsoR = sprt_Player_Morph;
					bodyFrame = clamp(8-scr_round(morphFrame), 0, 4);
					yOffFrame = bodyFrame;
				}
				else
				{
			...
			case AnimState.Jump:
			{
				...
				var jLiqMult = _liqMult * (2 - _liqMult);
				...
				if(aimAnimTweak > 0)
				{
					...
					if(!roomTrans)
					{
						if(velY <= 0)
						{
							frame[Frame.JAim] = max(frame[Frame.JAim] - 0.3 * jLiqMult, 0);
							frame[Frame.Jump] = 0;
						}
						else
						{
							...
							{
								frame[Frame.JAim] = min(frame[Frame.JAim] + max(1/max(frame[Frame.JAim],1),0.25) * jLiqMult, jAimFMax);
							}
							else if(bodyFrame <= 0)
							{
								frame[Frame.JAim] = max(frame[Frame.JAim] - 1*jLiqMult, jAimFMax);
							}
				...
				else
				{
					if(ledgeFall2)
					{
						...
						if(frameCounter[Frame.Jump] < 30)
						{
							if(!roomTrans)
							{
								frame[Frame.Jump] = min(frame[Frame.Jump] + max(0.5/max(frame[Frame.Jump],1),0.125)*jLiqMult, 4);
								if(frame[Frame.Jump] >= 4)
								{
									frameCounter[Frame.Jump] += 1 * _liqMult;
								}
							}
						...
						else
						{
							if(!roomTrans)
							{
								frame[Frame.Jump] = clamp(frame[Frame.Jump] + 0.25*jLiqMult, 4, 8);
							}
					...
					else
					{
						if(!roomTrans)
						{
							...
							{
								frame[Frame.Jump] = min(frame[Frame.Jump] + (0.5 * _liqMult), 4);
								frame[Frame.JAim] = 6;
							}
							else
							{
								frame[Frame.Jump] = clamp(frame[Frame.Jump] + (0.25 * _liqMult), 4, 9);
								frame[Frame.JAim] = 6;
							}
						}
			...
			case AnimState.Somersault:
			{
				...
				frame[Frame.JAim] = 6 * (velY <= 0);
				if(frame[Frame.WallJump] > 0 || (canWallJump && self.entity_place_collide(-8*move,0) && !self.entity_place_collide(0,16) && wjAnimDelay <= 0))
				{
					...
					if(frame[Frame.WallJump] > 0)
					{
						bodyFrame = wjSequence[scr_round(frame[Frame.WallJump])];
						frame[Frame.Somersault] = 3;
						if(item[Item.SpaceJump] && liquidState == LiquidState.None)
						{
							frame[Frame.Somersault] = 2;
						}
						...
						else if(liquidState == LiquidState.HydroBoost)
						{
							_animSpd = 0.915;
						}
						frame[Frame.WallJump] = max(frame[Frame.WallJump] - _animSpd, 0);
					}
					else
					{
						bodyFrame = 0;
				...
				else
				{
					...
					if(oldDir == dir)
					{
						...
						if(frameCounter[Frame.Somersault] >= num)
						{
							...
							frameCounter[Frame.Somersault] = max(frameCounter[Frame.Somersault] - num, 0);
						}
			...
			case AnimState.Grip:
			{
				...
				else
				{
					...
					if(climbIndexCounter >= 1/_liqMult)
					{
						climbFrame = climbSequence[climbIndex];
					}
			...
			case AnimState.Dodge:
			{
				...
				if(dodgeType == 1 && self.DodgeDir_Hori())
				{
					...
					{
						frame[Frame.Dodge] = min(frame[Frame.Dodge] + (0.5 * _liqMult), 2);
						if(groundedDodge == 1)
						{
					...
					{
						frame[Frame.Dodge] = max(frame[Frame.Dodge] - (1 * _liqMult), 0);
					}
				...
				{
					var _dAngle = scr_wrap(dodgeDir-180,-180,180);
					if(abs(_dAngle) == 180)
					{
						_dAngle = abs(_dAngle)*-dir;
					}
					var _fNum = abs(_dAngle / 45),
						_fNum2 = max(_fNum, 2);
					if(dodgeLength <= dodgeLengthMax-2)
					{
						if(frame[Frame.Dodge] < _fNum)
						{
							torsoR = sprt_Player_SomersaultRight;
							torsoL = sprt_Player_SomersaultLeft;
							bodyFrame = 1;
							if(frame[Frame.Dodge] > 0)
							{
								bodyFrame = 2 + 2*frame[Frame.Dodge];
							}
							self.SetArmPosSomersault(18, 40, bodyFrame);
							
							frame[Frame.Dodge] = min(frame[Frame.Dodge] + (1 * _liqMult), _fNum);
						}
						else
						{
							rotation = _dAngle;
							...
							bodyFrame = frame[Frame.Dodge]-_fNum;
							self.SetArmPosSpark(rotation);
							
							frameCounter[Frame.Dodge] += 1;
							if(!roomTrans || frameCounter[Frame.Dodge] > 4)
							{
								var _fSpd = (1 * _liqMult);
								if(roomTrans)
								{
									_fSpd = 1;
								}
								if(frame[Frame.Dodge] < _fNum+1)
								{
									frame[Frame.Dodge] = min(frame[Frame.Dodge] + _fSpd, _fNum+1);
								}
								else
								{
									frame[Frame.Dodge] = scr_wrap(frame[Frame.Dodge] + _fSpd, _fNum+1, _fNum+16);
								}
								frameCounter[Frame.Dodge] = 0;
							}
						}
						...
						if(groundedDodge == 1)
						{
							sprtOffsetY = -5*max(2-frame[Frame.Dodge],0);
						}
					}
					else
					{
						frame[Frame.Dodge] = clamp(frame[Frame.Dodge] - (1 * _liqMult), 0, 1);
						
						torsoR = sprt_Player_SomersaultRight;
						torsoL = sprt_Player_SomersaultLeft;
						bodyFrame = 2 - frame[Frame.Dodge];
						rotation = _dAngle;
						self.SetArmPosSomersault(18, 40, bodyFrame);
						
						var _angle = scr_wrap(-_dAngle*dir,0,360);
						var _somerFrame = _angle / (360 / 16);
						if(item[Item.SpaceJump] && liquidState == LiquidState.None)
						{
							_somerFrame = _angle / (360 / 8);
						}
			...
			case AnimState.Grapple:
			{
	            ...
	            else
	            {
	                ...
	                {
	                    grapFrame = min(grapFrame+_gFrNum,3);
	                }
	                if(grapFrame <= 2)
	                {
			...
			case AnimState.Hurt:
			{
				...
				bodyFrame = floor(frame[Frame.Hurt]);
				frame[Frame.Hurt] = min(frame[Frame.Hurt] + 0.34, 1);
				self.ArmPos(11,-11);
				if(dir == -1)
				{
					self.ArmPos(-2,-14);
			...
			case AnimState.DmgBoost:
			{
				...
				if(frame[Frame.DmgBoost] < 19)
				{
					bodyFrame = dBoostFrameSeq[frame[Frame.DmgBoost]];
					if(frame[Frame.DmgBoost] != 0)
					{
						torsoR = sprt_Player_SomersaultRight;
						torsoL = sprt_Player_SomersaultLeft;
					}
				...
				frameCounter[Frame.DmgBoost]++;
				if(frameCounter[Frame.DmgBoost] > 5)
				{
					frame[Frame.DmgBoost] = scr_wrap(frame[Frame.DmgBoost]+1, min(frame[Frame.DmgBoost],2), loopPoint);
					frameCounter[Frame.DmgBoost] = 6;
				}
	...
	if(!roomTrans)
	{
		...
		if(morphFrame <= 0 && frame[Frame.Crouch] <= 0)
		{
			aimAnimDelay = max(aimAnimDelay - 1, 0);
		}
		if(frame[Frame.WallJump] <= 0 || animState != AnimState.Somersault)
		{
			wjGripAnim = false;
		}
		...
		else if(animState == AnimState.Stand)
		{
			...
			if(!walkState)
			{
				walkToStandFrame = max(walkToStandFrame - (1/(1+(walkToStandFrame < 2))) * animSpeed, 0);
			}
		}
		...
		if(state != State.DmgBoost && animState != AnimState.DmgBoost)
		{
			frame[Frame.DmgBoost] = 0;
			frameCounter[Frame.DmgBoost] = 0;
		}
...
if(global.pauseState == PauseState.None)
{
	...
	else if(state != State.Grip)
	{
		...
		else if(!grounded && item[Item.SpaceJump] && state == State.Somersault && liquidState == LiquidState.None)
		{
			drawAfterImage = true;
			afterImageNum = 10;
		}
	...
	if(animState == AnimState.Walk || animState == AnimState.MoonPrep || animState == AnimState.Run || animState == AnimState.Jump || animState == AnimState.Somersault ||
	...
	if(animState == AnimState.Brake)
	{
		...
		var _bFrame = clamp(bodyFrame, 0, 4);
	...
	var canShoot = (currentWeapon.CanShoot() && dir != 0 && !startClimb && !moonFallState && !isPushing && state != State.Spark && state != State.BallSpark && 
					state != State.Hurt && (state != State.DmgBoost || animState != AnimState.DmgBoost) && state != State.Dodge && state != State.Death);
	if(!canShoot)
	{
		enqueShot = false;
	}
	...
	var killSnd = true;
	if(self.CanCharge() && cFire && dir != 0 && !isPushing && state != State.Death)
	{
		...
		if(cflag)
		{
			...
			if(statCharge >= 10)
			{
				...
				{
					if(audio_is_playing(snd_Charge) && animState == AnimState.DmgBoost && statCharge >= maxCharge)
					{
		...
		if(canShoot)
		{
			...
			else
			{
				...
				if(self.CanCharge())
				{
					...
					else if(statCharge >= delay && shotDelayTime <= 0)
					{
						self.PlayerShoot(shotInd,damage,sSpeed,delay,shotAmt,sndInd,beamIsWave,beamWaveStyleOffset);
						currentWeapon.OnShoot();
					}
