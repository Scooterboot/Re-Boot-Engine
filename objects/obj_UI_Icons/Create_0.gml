...
buttonIconKey[INPUT_VERB.EquipToggle] =	"${EquipToggle";//}";
...
function GetButtonIcons()
{
	for(var i = 0; i < INPUT_VERB._Length; i++)
	{
		for(var j = 0; j < _MAX_VERB_ICON_ALTS; j++)
		{
			...
			{
				buttonIcon[i][j] = UI_GetInputIconString(icon);
				
				// TODO: Maybe don't directly combine the mouse icon
				//		 and instead do something more dynamic?
				if(InputPlayerGetDevice(0) == INPUT_KBM)
				{
					var mouseIcon = undefined;
					if(i == INPUT_VERB.MenuAccept)
					{
						mouseIcon = InputIconGetDirect(mb_left, false, INPUT_KBM);
					}
					if(i == INPUT_VERB.MenuCancel)
					{
						mouseIcon = InputIconGetDirect(mb_right, false, INPUT_KBM);
					}
					
					if(!is_undefined(mouseIcon))
					{
						buttonIcon[i][j] += "/"+UI_GetInputIconString(mouseIcon);
					}
				}
			}
		}
	...
	for(var j = 0; j < _MAX_VERB_ICON_ALTS; j++)
	{
		if(is_undefined(buttonIcon[INPUT_VERB.RadialUIOpen][j]))
		{
	...
	for(var i = 0; i < INPUT_CLUSTER._Length; i++)
	{
		...
		for(var k = 0; k < _MAX_VERB_ICON_ALTS; k++)
		{
			...
			for(var j = 0; j < array_length(_verbs); j++)
			{
				...
				if(!is_undefined(icon))
				{
					str += UI_GetInputIconString(icon);
			...
			var str2 = self.AbbreviateCluster(_verbs, k);
...
function AbbreviateCluster(_verbs, _alt)
{
		...
		!is_undefined(icon[3]))
	{
		...
		if (icon[0] == InputIconGetDirect(gp_padu,_forGamepad,_gamepadType) &&
			icon[1] == InputIconGetDirect(gp_padd,_forGamepad,_gamepadType) &&
			icon[2] == InputIconGetDirect(gp_padl,_forGamepad,_gamepadType) &&
			icon[3] == InputIconGetDirect(gp_padr,_forGamepad,_gamepadType))
		{
			return "["+sprite_get_name(sprt_UI_GPIcon_DPad)+",0]";
		}
		
		if (icon[0] == InputIconGetDirect(-gp_axislv,_forGamepad,_gamepadType) &&
			icon[1] == InputIconGetDirect(gp_axislv,_forGamepad,_gamepadType) &&
			icon[2] == InputIconGetDirect(-gp_axislh,_forGamepad,_gamepadType) &&
			icon[3] == InputIconGetDirect(gp_axislh,_forGamepad,_gamepadType))
		{
			return "["+sprite_get_name(sprt_UI_GPIcon_LStick)+",0]";
		}
		
		if (icon[0] == InputIconGetDirect(-gp_axisrv,_forGamepad,_gamepadType) &&
			icon[1] == InputIconGetDirect(gp_axisrv,_forGamepad,_gamepadType) &&
			icon[2] == InputIconGetDirect(-gp_axisrh,_forGamepad,_gamepadType) &&
			icon[3] == InputIconGetDirect(gp_axisrh,_forGamepad,_gamepadType))
		{
			return "["+sprite_get_name(sprt_UI_GPIcon_RStick)+",0]";
		}
		
		if (icon[0] == InputIconGetDirect(vk_up,_forGamepad,_gamepadType) &&
			icon[1] == InputIconGetDirect(vk_down,_forGamepad,_gamepadType) &&
			icon[2] == InputIconGetDirect(vk_left,_forGamepad,_gamepadType) &&
			icon[3] == InputIconGetDirect(vk_right,_forGamepad,_gamepadType))
		{
			return "["+sprite_get_name(sprt_UI_KBIcon_ArrowKeyCluster)+",0]";
		}
	}
	return undefined;
}
...
{
	var str2 = str;
	str2 = self.InsertButtonIcons(str2);
	return str2;
}
